...
const Register = () => {
  const [step, setStep] = useState('input'); // 'input' | 'email-details' | 'phone-otp'
  const [identifier, setIdentifier] = useState('');
  const [inputType, setInputType] = useState(null); // null | 'email' | 'phone'
  ...
  // Step 1: Continue with email or phone
            ...
            <div>
              ...
              {identifier.trim() && (
                <p className="text-xs mt-1.5 text-gray-400">
                  {inputType === 'email' && 'Detected: Email address'}
                  {inputType === 'phone' && 'Detected: Phone number'}
                  {!inputType && 'Enter a valid email or phone number'}
                </p>
              )}
            </div>
        ...
        {step === 'email-details' && (
          <form onSubmit={handleEmailSubmit} className="space-y-5">
            ...
            <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5">
              ...
              <span className="text-sm text-gray-600 truncate">{identifier.trim()}</span>
            ...
            <div>
              ...
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required className={inputStyle} placeholder="At least 6 characters" />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Confirm Password</label>
              <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required className={inputStyle} placeholder="Confirm your password" />
            </div>
